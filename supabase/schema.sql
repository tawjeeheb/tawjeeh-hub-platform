...
-- ============================================================================
-- مركز توجيه — Career platform: majors, jobs, saved jobs, applications
-- (Phase 3 Persistence Foundation). Catalog is public-read; user rows are
-- per-user via RLS. The app falls back to seed data when these are empty.
-- ============================================================================
...
create index jobs_posted_idx on public.jobs (posted_at desc);
create index jobs_city_idx on public.jobs (city);
create index jobs_accepted_majors_idx on public.jobs using gin (accepted_majors);
